...
export const TransactionApprovalOwnershipRole = (props: any) => {
    ...
    const readServiceChangeDatas = async (id: number) => {
        ...
        const mailRoleParam = {
            Procedure: "FRM_MASTER.SERVICE_TYPE_ROLES_SPR",
            UserId: userID,
            CultureId: lang,
            Criteria: [{
                Name: "@ID",
        ...
        try {
            const [roleOptionList, roleList] =
                await axios.all([
                    ApiService.httpPost(API.masters.getreadRoles, roleParams),
                    ApiService.httpPost('data/getTable', mailRoleParam),
    ...
    const addNewRole = (formData: any) => {
        console.log(formData, roleOptionList)
        ...
        if (existingRole) {
            toast.error(`${newRoleName} ${t("already exists in the roles list")}`)
            return;
        }
        const newRole: IRole = {
            ID_: newRoleId,
    ...
    const onClickSaveRoleData = async () => {
        if (isValidCheck === 1 && getValues('serviceType')) {
            ...
            if (cnfrmDialog) {
                const response = await ApiService.httpPost('servicetypemails/saveRoles', saveRoleParam);
                if (response.Id > 0) {
    ...
    const callGridList = async () => {
        ...
        const response = await ApiService.httpPost('data/getTable', mailRoleParam);
        if (response.Valid > 0) {
            setSelectionChangeResponse({
                                ...
                                <TabContext value={tabValue}>
                                    <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                                        <TabList onChange={handleChangeTab} aria-label="lab API tabs example">
                                            <Tab label="Role List" value="1" />
                                            <Tab label="Additional Role List" value="2" />
                                        </TabList>
                                    </Box>
